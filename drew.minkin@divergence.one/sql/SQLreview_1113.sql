...
AS
	...
	SELECT
			sc.StudentID
			,SUM(c.CourseUnits) QtyUnits
			,COUNT(sc.CourseID) QtyEnrollments
		FROM
			dbo.Courses c
		JOIN
			[dbo].[StudentCourses] sc
...
UPDATE Students
SET EnrollmentStatus = se.EnrollmentStatus --where 
FROM Students s
...
--append in Power Query, same in DAX
UNION
		SELECT 
			'Tuition' as CostType
			,ebc.StudentID
